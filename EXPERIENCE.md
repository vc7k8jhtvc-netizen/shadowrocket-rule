...
Shadowrocket 以 `Shadowrocket_Routing_v2.7.0.conf` 为当前轻量路径：
...
- Routing v2.7.0 的 `[General]` 只能保留 `include = WestData.conf`，不得重新复制 DNS、TUN、Host、Rewrite、MITM 等基础设置。
...
- Shadowrocket v2.7.0 的业务策略组与规则应与 Clash 保持语义一致。
