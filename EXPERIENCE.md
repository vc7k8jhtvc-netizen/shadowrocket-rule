...
| 修改内容 | 维护要求 |
|---|---|
| 未分类国际服务域名 | 不再单独维护域名清单；中国规则与 GEOIP 未命中后，最终兜底规则直接进入 🌍 国际兜底 |
...
- 分流顺序：LAN → AI 专项例外 → Advertising → 其他专项服务 → China / China_Domain → GEOIP → 🌍 国际兜底。AI 在 Google 之前，字节跳动大陆直连规则在 TikTok 之前。
- 最后的 Shadowrocket `FINAL` / Clash `MATCH` 规则直接指向 🌍 国际兜底。
