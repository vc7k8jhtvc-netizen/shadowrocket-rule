...
try{
 ...
 const readmePath=path.join(temp,'README.md'),readme=fs.readFileSync(readmePath,'utf8');
 fs.writeFileSync(readmePath,readme.replace('内部版本为 `v2.7.10`','内部版本为 `v9.9.9`'));
 check('check-version.js',[],1,'version mismatch');
 fs.writeFileSync(readmePath,readme);
 const changelogPath=path.join(temp,'CHANGELOG.md'),changelog=fs.readFileSync(changelogPath,'utf8');
 fs.writeFileSync(changelogPath,changelog.replace('内部版本升至 `v2.7.10`','内部版本升至 `v9.9.9`'));
 check('check-version.js',[],1,'version mismatch');
 console.log('PASS: validator regressions reject missing DeepSeek DIRECT and version drift');
